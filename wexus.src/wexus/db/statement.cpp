...
/*bool statement::get_int(const std::string &colname, dbint &out) const
{
  short colnum;

  if (!find_colnum(colname, colnum))
    assert(false && "[cant find colname]\n");
  return get_int(colnum, out);
}

bool statement::get_double(const std::string &colname, dbdouble &out) const
{
  short colnum;

  if (!find_colnum(colname, colnum))
    assert(false && "[cant find colname]\n");
  return get_double(colnum, out);
}

bool statement::get_string(const std::string &colname, dbstring &out) const
{
  short colnum;

  if (!find_colnum(colname, colnum))
    assert(false && "[cant find colname]\n");
  return get_string(colnum, out);
}

bool statement::get_date(const std::string &colname, dbdate &out) const
{
  short colnum;

  if (!find_colnum(colname, colnum))
    assert(false && "[cant find colname]\n");
  return get_date(colnum, out);
}

bool statement::get_timestamp(const std::string &colname, dbtimestamp &out) const
{
  short colnum;

  if (!find_colnum(colname, colnum))
    assert(false && "[cant find colname]\n");
  return get_timestamp(colnum, out);
}

dbint statement::get_int(const std::string &colname) const
{
  dbint ret;
  short colnum;
  ...
  if (!find_colnum(colname, colnum))
    return 0;
  if (!get_int(colnum, ret))
    return 0;
  return ret;
}

dbdouble statement::get_double(const std::string &colname) const
{
  dbdouble ret;
  short colnum;
  ...
  if (!find_colnum(colname, colnum))
    return 0;
  if (!get_double(colnum, ret))
    return 0;
  return ret;
}

dbstring statement::get_string(const std::string &colname, int stringbufsize) const
{
  dbstring ret(stringbufsize);
  short colnum;

  if (!find_colnum(colname, colnum))
    return ret;
  get_string(colnum, ret); // fail or not, we're returning ret
  return ret;
}

dbdate statement::get_date(const std::string &colname) const
{
  dbdate ret;
  short colnum;
  ...
  if (!find_colnum(colname, colnum))
    return ret;
  get_date(colnum, ret); // fail or not, we're returning ret
  return ret;
}

dbtimestamp statement::get_timestamp(const std::string &colname) const
{
  dbtimestamp ret;
  short colnum;
  ...
  if (!find_colnum(colname, colnum))
    return ret;
  get_timestamp(colnum, ret); // fail or not, we're returning ret
  return ret;
}*/
